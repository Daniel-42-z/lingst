...
#let lingo-pipe-puzzle(
	...
	for i in range(1, height-color-list.len()) {
		let curr = parse-height(height-color-list.at(i).at(0))
		let prev = parse-height(height-color-list.at(i - 1).at(0))
	...
	let cells = range(layout.len()).map(i => {
		...
		let (h, color) = (parse-height(item.at(0)), item.at(1))
